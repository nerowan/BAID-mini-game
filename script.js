...
// 技能卡库
const skillCardPool = [
    { name: '射门大师', effect: { shooting: 5 }, description: '射门+5' },
    { name: '防守墙', effect: { defense: 5 }, description: '防守+5' },
    { name: '速度之星', effect: { speed: 5 }, description: '速度+5' },
    { name: '盘带高手', effect: { dribbling: 5 }, description: '盘带+5' },
    { name: '传球大师', effect: { passing: 5 }, description: '传球+5' },
    { name: '全能选手', effect: { shooting: 2, defense: 2, speed: 2, dribbling: 2, passing: 2 }, description: '全属性+2' },
];
...
function updateFundsDisplay() {
    document.getElementById('funds').innerText = `资金：${funds}金币`;
}
    ...
    if (playerList) {
        ...
        teamPlayers.forEach(player => {
            if (!startingPlayers.includes(player.name) && !player.suspended && !injuredPlayers.includes(player.name)) {
                ...
                li.className = `player ${getCardClass(player.card)}`;
                li.innerHTML = `
                    <div>${player.name}</div>
...
function showPlayerCard(player) {
    ...
    if (player.skills.length > 0) {
        ...
        equippedSkillsList.innerHTML = '<li>未装备技能卡</li>';
    }

    document.getElementById('renew-contract-btn').onclick = () => renewContract(player);
    document.getElementById('motivate-player-btn').onclick = () => motivatePlayer(player);
    document.getElementById('talk-to-player-btn').onclick = () => talkToPlayer(player);
    document.getElementById('sell-player-btn').onclick = () => sellPlayer(player);
    document.getElementById('equip-skill-btn').onclick = () => equipSkillToPlayer(player);
}

// 续约功能
function renewContract(player) {
    const cost = player.card === '钻石卡' ? 300000 : player.card === '金卡' ? 200000 : player.card === '银卡' ? 100000 : 50000;
    if (funds >= cost) {
        funds -= cost;
        updateFundsDisplay();
        player.contract += 1;
...
// 训话功能
function motivatePlayer(player) {
...
function talkToPlayer(player) {
    $('#player-dialogue-modal').modal('show');
    ...
    options.forEach(option => {
        ...
        btn.className = 'btn btn-block btn-primary mb-2';
        ...
        btn.onclick = () => {
            ...
            $('#player-dialogue-modal').modal('hide');
            $('#player-card-modal').modal('hide');
        };
...
// 装备技能卡给球员
function equipSkillToPlayer(player) {
    $('#equip-skill-modal').modal('show');
    ...
    if (ownedSkillCards.length === 0) {
        skillOptions.innerHTML = '<p>你没有任何技能卡。</p>';
        return;
    }
    ...
    ownedSkillCards.forEach(skill => {
        const btn = document.createElement('button');
        btn.className = 'btn btn-block btn-primary mb-2';
        btn.innerText = `${skill.name} (${skill.description})`;
        btn.onclick = () => {
            // 移除旧的技能卡效果
            if (player.skills.length > 0) {
    ...
    positions.forEach(pos => {
        ...
        if (currentStartingLineup[pos.position]) {
            ...
            if (player) {
                ...
                playerCard.addEventListener('click', () => showPlayerCard(player));
...
function slotDrop(event) {
    ...
    const playerName = draggedPlayer.dataset.playerName;
    const player = getPlayerByName(playerName);

    // 防止同一个球员被多次添加
    if (Object.values(currentStartingLineup).includes(playerName) && currentStartingLineup[positionKey] !== playerName) {
        alert('该球员已在阵容中！');
        return;
    }
    ...
    if (draggedPlayer.parentElement.classList.contains('position-slot')) {
        ...
        if (currentStartingLineup[positionKey]) {
            ...
            if (replacedPlayerName !== '虚拟球员') {
                const replacedPlayer = getPlayerByName(replacedPlayerName);
                if (replacedPlayer) {
                    loadTeamPlayers();
                }
            }
...
playerListContainer.addEventListener('drop', event => {
    event.preventDefault();
    if (draggedPlayer && draggedPlayer.parentElement.classList.contains('position-slot')) {
...
document.getElementById('auto-config-btn').addEventListener('click', () => {
    ...
    // 优先匹配位置相同的球员
    positions.forEach(pos => {
        const player = teamPlayers.find(p => p.position === pos.position && !Object.values(currentStartingLineup).includes(p.name) && !p.suspended && !injuredPlayers.includes(p.name));
        if (player) {
    ...
    positions.forEach(pos => {
        if (!currentStartingLineup[pos.position]) {
            const player = teamPlayers.find(p => !Object.values(currentStartingLineup).includes(p.name) && !p.suspended && !injuredPlayers.includes(p.name));
            if (player) {
                currentStartingLineup[pos.position] = player.name;
            } else {
                // 如果没有球员可用，使用虚拟球员填充
...
function startMatch() {
    ...
    // 生成比赛天气
    const weatherOptions = ['晴天', '小雨', '大雨', '多云', '阴天', '雪天'];
...
function generateMatchEvent(stats, unavailablePlayers) {
    ...
    switch (eventType) {
        ...
        case '受伤':
            ...
            break;
        case '严重犯规':
            if (!unavailablePlayers.includes(player.name) && Math.random() < 0.05) {
                stats.fouls += 1;
                stats.redCards += 1;
                unavailablePlayers.push(player.name);
                eventText = `${player.name}严重犯规，被终身禁赛！`;
...
function updateMatchStatsDisplay(stats) {
    document.getElementById('match-time').innerText = `比赛时间：${stats.time}'`;
    document.getElementById('score').innerText = `比分：${stats.score.team} - ${stats.score.opponent}`;
...
function checkAchievements() {
    ...
    // 检查钻石球员数量
    const diamondPlayers = teamPlayers.filter(player => player.card === '钻石卡');
...
const allPlayers = [
    ...
    { name: '球员H', position: 'GK', card: '钻石卡', contract: 3, value: 500000, overall: 88, stamina: 100, suspended: false, yellowCards: 0, skills: [], stats: { shooting: 50, defense: 90, speed: 70, dribbling: 65, passing: 80 } },
    // 添加更多球员
    ...
    { name: '球员L', position: 'CB', card: '钻石卡', contract: 3, value: 500000, overall: 90, stamina: 100, suspended: false, yellowCards: 0, skills: [], stats: { shooting: 70, defense: 95, speed: 80, dribbling: 75, passing: 80 } },
    ...
    { name: '球员N', position: 'LW', card: '钻石卡', contract: 3, value: 500000, overall: 91, stamina: 100, suspended: false, yellowCards: 0, skills: [], stats: { shooting: 85, defense: 65, speed: 85, dribbling: 85, passing: 80 } },
...
function drawCard(packType) {
    ...
    if (funds >= cost) {
        funds -= cost;
        updateFundsDisplay();
        const availablePlayers = allPlayers.filter(p => p.card === packCardType);
        const randomPlayer = availablePlayers[Math.floor(Math.random() * availablePlayers.length)];
        alert(`恭喜你获得了${packCardType}：${randomPlayer.name}`);
...
function loadTransferMarket() {
    ...
    transferPlayers.forEach(player => {
        const li = document.createElement('li');
        li.className = `transfer-player ${getCardClass(player.card)} d-flex flex-column align-items-center`;
        li.innerHTML = `
            <div>${player.name}</div>
            <div>${player.position} - ${player.card}</div>
            <div>综合能力：${player.overall}</div>
            <div>估价：${player.value}金币</div>
            <button class="negotiate-player-btn btn btn-sm btn-primary mt-2">谈判</button>
        `;
        li.querySelector('.negotiate-player-btn').addEventListener('click', () => negotiatePlayer(player));
        list.appendChild(li);
    });
}
...
function negotiatePlayer(player) {
    ...
    negotiationContent.innerHTML = `
        ...
        <input type="number" id="offer-input" class="form-control" placeholder="请输入您的报价">
        <p id="negotiation-feedback" class="text-danger"></p>
    `;
    document.getElementById('offer-btn').onclick = () => makeOffer(player);
    document.getElementById('cancel-negotiation-btn').onclick = () => $('#negotiation-modal').modal('hide');
}
...
function loadCoachMarket() {
    ...
    coachMarket.forEach(coach => {
        const li = document.createElement('li');
        li.className = `coach-card ${getCardClass(coach.card)} d-flex flex-column align-items-center`;
        li.innerHTML = `
            <div>${coach.name}</div>
            <div>等级：${coach.level}</div>
            <div>价格：${coach.price}金币</div>
            <div>加成：射门+${coach.bonus.shooting} 防守+${coach.bonus.defense} 速度+${coach.bonus.speed} 盘带+${coach.bonus.dribbling} 传球+${coach.bonus.passing}</div>
            <button class="hire-coach-btn btn btn-sm btn-primary mt-2">签约</button>
        `;
        li.querySelector('.hire-coach-btn').addEventListener('click', () => hireCoach(coach));
        list.appendChild(li);
    });
}
...
function loadTraining() {
    ...
    teamPlayers.forEach(player => {
        const li = document.createElement('li');
        li.className = `player ${getCardClass(player.card)} d-flex flex-column align-items-center`;
        li.innerHTML = `
            <div>${player.name}</div>
            <div>${player.position} - ${player.card}</div>
            <div>综合能力：${player.overall}</div>
            <div>体力：${player.stamina}</div>
            <button class="btn btn-sm btn-primary mt-2">训练</button>
        `;
...
function handleInjuriesAndSuspensions() {
    // 这里可以添加受伤球员的恢复逻辑
    injuredPlayers = []; // 简化处理，受伤球员一场比赛后恢复
}
...
function loadYouthAcademy() {
    ...
    youthPlayers.forEach(player => {
        ...
        li.innerHTML = `
            ...
            <button class="btn btn-sm btn-primary mt-2">提升至一线队</button>
        `;
    ...
    const events = [
        ...
        },
        {
            ...
            effect: () => {
                ...
                alert(`球员${injuredPlayer.name}受伤，暂时无法上场。`);
                loadTeamPlayers();
                loadPlayerInfo();
                generatePitch();
            },
...
function loadSkillCards() {
    ...
    ownedSkillCards.forEach(skill => {
        const li = document.createElement('li');
        li.className = 'skill-card-item d-flex flex-column align-items-center';
        li.innerHTML = `
            <div>${skill.name}</div>
